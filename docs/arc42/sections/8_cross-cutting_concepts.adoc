...
image::adapter_wiremock_mode.png[PIS child tree,width=100%]
...
XS2A Adapter developed its own set of exceptions that can occur in specific situations. All of them are subtype of RuntimeException.

.XS2A Adapter Exceptions
image::adapter-exceptions.png[PIS child tree,width=100%]
...
|AspspRegistrationNotFoundException |Thrown if no adapter found in the Registry by provided parameters (IBAN, DIC, Bank Code, etc.) |400
|BadRequestException |Thrown if malformed request received, used in specific case like missing scope for OAuth or wrong body
...
|OAuthException |Thrown if OAuth2 service failed to complete an operation. |403
